  ...
  <div class="profile-page">
    ...
    <div v-if="tab === 'fav'" class="list-section">
      <div v-if="favList.length" class="list-toolbar">
        <span class="list-count mono">{{ favList.length }} items</span>
    ...
    <div v-if="tab === 'history'" class="list-section">
      <div v-if="histList.length" class="list-toolbar">
        <span class="list-count mono">{{ histList.length }} items</span>
...
.username {
  font-family: 'Noto Serif SC', serif;
  font-size: 17px; font-weight: 700; color: var(--text-primary); margin-bottom: 5px;
}
...
.tag.mono { font-family: 'DM Mono', monospace; font-size: 10px; }
...
.list-count.mono { font-family: 'DM Mono', monospace; letter-spacing: 0.5px; }
.danger-text { font-size: 12px; color: #F85149; font-weight: 600; }

.empty-state { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 48px 0; }
.empty-state p { font-size: 13px; color: var(--text-muted); font-family: 'DM Mono', monospace; letter-spacing: 1px; }
...
.list-item {
  ...
  margin-bottom: 6px; padding: 12px; display: flex;
...
.item-meta span { font-family: 'DM Mono', monospace; font-size: 10px; color: var(--text-muted); }
.sep { color: var(--border-strong); }
.del-btn {
  color: var(--text-muted); padding: 4px; flex-shrink: 0;
  display: flex; align-items: center; transition: color 0.15s;
}
.del-btn:active { color: #F85149; }
...
.sheet-handle {
  width: 36px; height: 3px; background: var(--border-strong);
  border-radius: 2px; margin: 4px auto 0; flex-shrink: 0;
}
.sheet-eyebrow {
  font-family: 'DM Mono', monospace; font-size: 10px;
  font-weight: 500; letter-spacing: 3px; color: var(--brand); margin-top: 4px;
}
.sheet-title {
  font-family: 'Noto Serif SC', serif;
  font-size: 18px; font-weight: 900; color: var(--text-primary); margin-top: -6px;
}
...
.sheet-field input:focus, .sheet-field textarea:focus {
...
.sheet-save {
  flex: 2; padding: 12px; background: var(--brand); color: var(--bg);
